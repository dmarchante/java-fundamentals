# Java Fundamentals


  ## Basic Folder
  - [Main Class](./basics/Main.java)

  ## Basic Library Folder
    ### Main
      - [Library Class](./basiclibrary/src/main/java/basiclibrary/Library.java)

    ### Test
      - [Library Test Class](./basiclibrary/src/test/java/basiclibrary/LibraryTest.java)

  ## Inheritance Folder
    ### Main
      - [Restaurant Class](./inheritance/src/main/java/inheritance/Restaurant.java)
      - [Review Class](./inheritance/src/main/java/inheritance/Review.java)
      - [Shop Class](./inheritance/src/main/java/inheritance/Shop.java)
      - [Theater Class](./inheritance/src/main/java/inheritance/Theater.java)
      - [Theater Review Class](./inheritance/src/main/java/inheritance/TheaterReview.java)

    ### Test
      - [Restaurant Test Class](./inheritance/src/test/java/inheritance/RestaurantTest.java)
      - [Review Test Class](./inheritance/src/test/java/inheritance/ReviewTest.java)
      - [Shop Test Class](./inheritance/src/main/java/inheritance/ShopTest.java)
      - [Theater Test Class](./inheritance/src/main/java/inheritance/TheaterTest.java)
      - [Theater Review Test Class](./inheritance/src/main/java/inheritance/TheaterTestReview.java)

  ## Linter Folder
    ### Main
      - [App Class (Linter)](./linter/src/main/java/linter/App.java)

    ### Test
      - [App Test Class (Linter)](./linter/src/test/java/linter/AppTest.java)

  ## Quote Folder
    ### Main
      - [App Class (Quote)](./quotes/src/main/java/quotes/App.java)
      - [Star Wars Quote Class](./quotes/src/main/java/quotes/StarWarsQuote.java)
      - [Quote Class](./quotes/src/main/java/quotes/Quote.java)

    ### Test
      - [App Test Class (Quote)](./quotes/src/test/java/quotes/AppTest.java)

  ## Songr Folder
    ### Main
      - [Songr Class](./songr/src/main/java/com.dmarchante.code401d4/Songr.java)
      - [Helllo World Controller](./songr/src/main/java/com.dmarchante.code401d4/HelloWOrldController.java)

    ### Test
      - [Songr Test Class](./songr/src/main/java/songr/SongrTest.java)

